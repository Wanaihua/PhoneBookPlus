...
<html>
  <head>
	...
	<style type="text/css">
	  ...
	  <%
		  int who=0;
		  if(request.getAttribute("who")!=null) who= Integer.parseInt(request.getAttribute("who").toString());
		  if(request.getParameter("who")!=null) who= Integer.parseInt(request.getParameter("who"));
	  %>
	<script type="text/javascript">
		function a(){
			window.open("registerKind.jsp?who=<%=who%>","_self");
		}
		function c(){
		...
		function e(){
			window.open("deleteKind.do?id=${kind.kindId}&amp;p=${page.currentPage}&amp;who=<%=who%>","_self");
		}
	</script>
  </head>
...
　		<tr align="center" class="column_${s.count }" height="50px">
		     ...
		     <td width="100px"><a href="deleteKind.do?id=${kind.kindId}&amp;p=${page.currentPage}&amp;who=<%=who%>"><spring:message code="delete"></spring:message></a></td>
		     <td width="100px"><a href="queryById.do?id=${kind.kindId}&amp;who=<%=who%>"><spring:message code="look"></spring:message></a></td>
		   </tr>
		 </c:forEach>
  ...
  <br>
  <a href="list.do?p=1&amp;who=<%=who %>"><spring:message code="FirstPage"></spring:message></a>
    <c:choose>  
           <c:when test="${page.currentPage>1}">
               <a href="list.do?p=${page.currentPage-1}&amp;who=<%=who %>"><spring:message code="PreviousPage"></spring:message></a>
           </c:when>  

    </c:choose>
    <c:choose>  
           <c:when test="${page.currentPage<page.totalPage}">
               <a href="list.do?p=${page.currentPage+1}&amp;who=<%=who %>"><spring:message code="NextPage"></spring:message></a>
           </c:when>  
    </c:choose>
     <a href="list.do?p=${page.totalPage}&amp;who=<%=who %>">
